...
export class DataSourceUtil {
  private static hasValidDataSource(path: string): boolean {
  ...
  private static async requestEntities<K extends keyof ApiEntities>(
    entitiesName: K,
    parameter?: string,
  ...
  public static async loadEntities<K extends keyof ApiEntities>(
    entitiesName: K,
    parameter?: string,
  ): Promise<ApiEntities[K][]> {
